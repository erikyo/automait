...
import { c, printError, printWarning } from "./ui.js";
...
program
	...
	.option(
		"-p, --system-prompt <string>",
		"Override the system prompt for commit generation",
	)
	.option(
		"-d, --dry-run",
		"Analyse and plan only — do not run any git commands",
		false,
	)
	.option(
		"--commit <count>",
		"Preferred total number of commits",
		parseCommitCount,
	)
...
async function main() {
	...
	console.log(
		c.muted(
			`  Using model: ${config.model}  |  endpoint: ${config.ollamaEndpoint}\n`,
		),
	);
