...
function extractThinkTags(content) {
  const thoughts = [];
  const cleaned = content.replace(/<think>([\s\S]*?)<\/think>/gi, (_, thought) => {
 ...
 */
export async function ollamaChat(baseUrl, model, messages, timeoutMs = 120_000) {
  ...
  return {
    ...
    thinking: typeof thought === 'string' && thought.trim() ? thought.trim() : null,
  };
}
...
function parseJsonResponse(text, label) {
  ...
  try {
    ...
    throw new Error(`Ollama returned non-JSON response for ${label}.\nExtracted response:\n${json}`);
  }
}
...
function normalizeFileSelection(selected, remaining) {
  ...
  const valid = normalizeStringArray(selected).filter((file) => remainingSet.has(file));
...
function fallbackSelection(fileIndex, analyzedFiles) {
  ...
  const firstDir = first.path.includes('/') ? first.path.split('/').slice(0, -1).join('/') : '';
...
function normalizeObservationResult(parsed) {
  return {
    observations: normalizeStringArray(parsed.observations || parsed.changes || parsed.summary),
...
function normalizeCommitPlan(parsed, fallbackSummary, allowedFiles, targetCommitCount) {
  ...
  const commits = rawCommits
    .map((commit, index) => {
      ...
      return {
        ...
        files,
        rationale: commit.rationale || commit.Rationale || commit.reason || commit.explanation || '',
      };
    })
    .filter((commit) => commit.files.length);

  if (!commits.length) {
    throw new Error('Commit plan is empty or malformed - no valid commits array found.');
  }
  ...
  if (missingFiles.length) {
    commits.push({
      ...
      files: missingFiles,
      rationale: 'Files were analyzed but omitted from the model commit grouping.',
    });
  }
...
function fallbackGitignoreSuggestions(fileIndex, gitignoreContent) {
  ...
  const rules = [
    { test: (path) => path === '.DS_Store' || path.endsWith('/.DS_Store'), pattern: '.DS_Store', reason: 'macOS metadata file' },
    { test: (path) => path.includes('node_modules/'), pattern: 'node_modules/', reason: 'npm dependency directory' },
    { test: (path) => path.includes('vendor/'), pattern: 'vendor/', reason: 'vendored dependency directory' },
    { test: (path) => path.includes('vendors/'), pattern: 'vendors/', reason: 'vendored dependency directory' },
    { test: (path) => path.includes('dist/'), pattern: 'dist/', reason: 'generated build output' },
    { test: (path) => path.includes('build/'), pattern: 'build/', reason: 'generated build output' },
    { test: (path) => path.endsWith('.log'), pattern: '*.log', reason: 'log file' },
  ];
  ...
  for (const rule of rules) {
    ...
    if (matched_files.length && !ignored.has(rule.pattern)) {
      suggestions.push({ pattern: rule.pattern, reason: rule.reason, matched_files });
    }
  }
...
function normalizeGitignoreSuggestions(parsed, fileIndex, gitignoreContent) {
  ...
  return rawSuggestions
    .map((item) => {
      ...
      const matched_files = normalizeStringArray(item.matched_files || item.files)
        .filter((file) => staged.has(file));

      return {
        pattern,
        reason: typeof item.reason === 'string' ? item.reason.trim() : 'Looks like generated or local-only content.',
        matched_files,
      };
    })
    .filter((item) => item.pattern && item.matched_files.length && !existing.has(item.pattern));
}
...
function mergeGitignoreSuggestions(...groups) {
  ...
  for (const group of groups) {
    for (const item of group) {
      ...
      existing.matched_files = [...new Set([
    ...
    commits: [
      {
        ...
        files,
        rationale: memory.knowledge.slice(0, 3).join('; ') || 'Grouped from staged diff analysis.',
      },
    ],
  };
}
...
- return strict JSON with this schema:
{
  ...
  let parsed = {};
  try {
    const { content, thinking } = await ollamaChat(baseUrl, model, messages, timeoutMs);
  ...
  const selected = normalizeFileSelection(parsed.selected || parsed.files, remaining);

  return {
    selected: selected.length ? selected : fallbackSelection(fileIndex, memory.analyzed_files),
    why: typeof parsed.why === 'string' ? parsed.why : '',
  };
}

async function analyzeSelectedFiles(baseUrl, model, selected, diff, memory, timeoutMs) {
  const messages = [
...
- return strict JSON:
{
  ...
  try {
    ...
    return normalizeObservationResult(parseJsonResponse(content, 'diff analysis'));
  } catch {
    return {
      observations: [`Inspected staged changes for ${selected.join(', ')}`],
      possible_commit_scope: selected[0]?.split('/')[0] || 'staged',
      notes_for_next_iteration: 'Diff analysis used a deterministic fallback after invalid model JSON.',
      themes: [],
      open_questions: [],
    };
  }
...
export async function generateCommitPlan(baseUrl, model, diff, timeoutMs, options = {}) {
  ...
  const trace = [];
  const maxIterations = Math.max(fileIndex.length * MAX_ITERATIONS_FACTOR, fileIndex.length);

  for (let iteration = 1; memory.analyzed_files.length < fileIndex.length; iteration++) {
    if (iteration > maxIterations) {
      throw new Error('Planner stopped after too many iterations without covering all staged files.');
    }

    const selection = await chooseNextFiles(baseUrl, model, fileIndex, memory, timeoutMs);
    const selected = selection.selected.filter((file) => !memory.analyzed_files.includes(file));
    ...
    if (!selectedDiff) {
      ...
      memory.open_questions.push(`No staged diff was available for ${selected.join(', ')}`);
      continue;
    }
    ...
    if (analysis.notes_for_next_iteration) memory.knowledge.push(analysis.notes_for_next_iteration);
    if (analysis.possible_commit_scope) memory.themes.push(analysis.possible_commit_scope);
  ...
  try {
    plan.reasoningTrace = await summarizeReasoning(baseUrl, model, memory, plan, timeoutMs);
  } catch {
